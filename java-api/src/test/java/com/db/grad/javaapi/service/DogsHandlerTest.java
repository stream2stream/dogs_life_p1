...
public class DogsHandlerTest
{
    ...
    public void add_a_dog_return_number_of_dogs_in_repo_is_one()
    {
        ...
        Mockito.when(itsDogsRepo.save(theDog))
                .thenReturn(theDog);
    ...
    public  void    add_dog_and_remove_dog_that_doess_not_exist_return_number_of_dogs_is_one()
    {
        ...
        Mockito.when(itsDogsRepo.findById((int) invalidId)).thenReturn(opt);

        Mockito.when(itsDogsRepo.count()).thenReturn(1L);

        // act
        // There is no dog with ID == 33
        boolean actualStatus = cut.removeDog((int) invalidId);
        long actualResult = cut.getNoOfDogs();
