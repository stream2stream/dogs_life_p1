...
{
    @Mock
    private DogsRepositoryJPA itsDogsRepo;
...
////    @Ignore
    ...
    public  void    add_dog_and_remove_dog_return_number_of_dogs_is_zero()
    {
        ...
        Mockito.when(itsDogsRepo.delete(theDog)).thenReturn(true);
...
//
    ...
    public  void    find_dog_by_valid_id_returns_one_dog()
    {
        ...
        // act
        Optional<Dog> actualResult = cut.getDogById( addedDog.getId() );

        // assert
        assertEquals( expectedDog.getId(), actualResult.map(Dog::getId).orElse(-1L));
        assertEquals( expectedDog.getName(), actualResult.map(Dog::getName).orElse(""));
    }
//
////    @Ignore
    @Test
